...
class CommunitypricecitycountymoreSpider(Spider):
    name = 'CommunityPriceCityCountyMore'
...
# 从txt文件中读取链接
    ...
    # 从json文件中读取链接
    f = codecs.open('citylinks_last_page.json', encoding='utf-8')
    str_value = f.read()
    ...
    def start_requests(self):
        for d in self.json_data:
            ...
            if city and url is not None:
                result = re.match('(.*)p50/', url)
                if result is not None:
                    the_link = result.group(1)
                    for num in range(51, 101):
                        link = the_link + 'p' + str(num) + '/'
